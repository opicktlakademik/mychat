...
function sendMessage()
{
  ...
  if (message) {
		$('#input_message').val('');
		console.log("Mengambil pesan dari type box");
		console.log("Pesan: "+message+"\n");
		
		var properties = pakde(0);
		console.log("\nMenyiapkan Properties Diffie Hellman");
		console.log("Properti 1: ");
		console.log(properties.key1_to_send);
		console.log("Properti 2: ");
		console.log(properties.key2_to_send);
		console.log("\n[MELAKUKAN TRANSAKSI DENGAN SERVER]");
		console.log("\n- Mengirim permintaan get untuk pertukaran kunci...");		
    ...
    $.get('Chat/ackReq', {key1:properties.key1_to_send, key2:properties.key2_to_send}, function(e, success){
      if (success) {
        ...
        var encmsg = pakde(1, {'message':message, 'enc1key':parseInt(enc1key), 'enc2key':parseInt(enc2key)});
				console.log(data);
				console.log("\n- Generate key pergeseran...");
        console.log("Kunci 1: "+enc1key);
				console.log("Kunci 2: "+enc2key);
				console.log("\n- Menyiapkan pesan untuk dikirim ke server");
        var date = new Date();
        ...
        var i = parseInt($('div.item').length);
				...
				console.log("Recipient ID: " + $('input[name=recipient_id]').val());
				console.log("message: ");
				console.log(encmsg);
